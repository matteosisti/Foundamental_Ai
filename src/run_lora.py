...
# ---- W&B ONLINE for live monitoring (key from Colab secret) ----
...
def _load_zarr_to_ram(zpath, name):
    ...
    n = imgs_z.shape[0]
    print('[' + name + '] loading ' + str(n) + ' samples into RAM...')
...
def load_coco_fuzzy(model, ckpt_path):
    ...
    print('[fuzzy] loaded ' + str(len(loadable)) + '/' + str(len(clean)) + ' keys')
...
# ---------- trainer (CSV + W&B online logging) ----------
def make_trainer(max_epochs, run_name):
    csv_logger = CSVLogger(save_dir=OUT_DIR, name='logs_' + run_name)
    wandb_logger = WandbLogger(project='eomt-lora-ablation', name=run_name, save_dir=OUT_DIR, offline=False)
...
dm.setup()
print('DataModule ready. Train:', len(dm.train_ds), 'Val:', len(dm.val_ds))
