...
def main():
	...
	logits_path = logits_dir / f"{args.dataset_name}__erfnet_logits_f32.npy"
	...
	logits_t = torch.from_numpy(logits)  # float32 on CPU
	...
	for T in T_list:
		...
		out_json = results_dir / f"{args.dataset_name}__ERFNet__msp__T{T}__from_cache__{args.mode}.json"
		with open(out_json, "w") as f:
			json.dump(metrics, f, indent=2)

		append_metrics_csv(results_dir / "metrics.csv", metrics)

		print(f"[T={T}] mode={args.mode} | AUPRC={auprc*100:.4f} | FPR95={fpr95*100:.4f} | saved {out_json}")
